...
# Helper para controlar a janela do terminal via Win32 API
$Win32Signature = @"
[DllImport("user32.dll")]
public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow);
"@
$Win32 = Add-Type -MemberDefinition $Win32Signature -Name "Win32ShowWindow" -Namespace Win32Functions -PassThru
...
try {
    ...
    while ($listener.IsListening) {
        ...
        # POST /minimize — minimiza a janela do terminal
        if ($req.HttpMethod -eq "POST" -and $req.Url.AbsolutePath -eq "/minimize") {
            try {
                $hwnd = (Get-Process -Id $PID).MainWindowHandle
                if ($hwnd -ne 0) {
                    $Win32::ShowWindow($hwnd, 2) # SW_MINIMIZE = 2
                    Write-JsonResponse -Response $res -Payload @{ success = $true; output = "Janela minimizada." }
                } else {
                    Write-JsonResponse -Response $res -Payload @{ success = $false; output = "Janela nao encontrada." } -StatusCode 500
                }
            } catch {
                Write-JsonResponse -Response $res -Payload @{ success = $false; output = "Erro ao minimizar: $($_.Exception.Message)" } -StatusCode 500
            }
            continue
        }
